...
module.exports = configure = deferred(function(action, params) {
  ...
  marshal = deferred(function(_arg, object) {
    var realzerFn, reject, resolve, _base, _base1, _base2;
    resolve = _arg.resolve, reject = _arg.reject;
    ...
    if (connect) {
      if (!((port != null) || ((object.connect != null) && (object.connect.port != null)))) {
        return reject(error({
          errno: 105,
          code: 'ENOPORT',
      ...
      if (port != null) {
        object.connect.port = port;
      }
      if (typeof hostname !== "undefined" && hostname !== null) {
        object.connect.hostname = hostname;
      }
      (_base = object.connect).hostname || (_base.hostname = 'localhost');
      if (https) {
        object.connect.transport = 'https';
      } else {
        (_base1 = object.connect).transport || (_base1.transport = 'http');
      }
      if (process.env.REALIZER_SECRET != null) {
        object.connect.secret = process.env.REALIZER_SECRET;
      } else {
        if ((_base2 = object.connect).secret == null) {
          _base2.secret = '';
        }
      }
    }
